...
b = rand() + 1;
lambda = 0.5;
[t,x] = ode45(@(t,x)(de(t,x,a,b,ahat,bhat,lambda)),tspan,x0);
%% plotting
xdot = (1-ahat)*x(:,2) + x(:,1);
% sliding variable
S = xdot  + lambda*(x(:,1));
...
u = 1/(1-a)*(-(1-ahat).*(bhat.*((xdot-x(:,1))./(1-a)).^2 - x(:,1)) - xdot - K.*sign(S)) ;
...
function xdot = de(t,x,a, b,ahat,bhat,lambda)
    xdot = zeros(2,1);
    xdot(1) = (1-ahat)*x(2) + x(1);
    S = xdot(1) + lambda*x(1);
    K = abs(-(1-ahat).*(bhat.*((xdot(1)-x(1))./(1-a))^2)-(1-a).*(b.*((xdot(1)-x(1))./(1-a))^2)) + 1;
    u = 1/(1-a)*(-(1-ahat).*(bhat.*((xdot(1)-x(1))./(1-a))^2 - x(1)) - xdot(1) - K*sign(S)) ;
